...
public class ServiceUtil {
	 ...
	 * @return
	 */
	public static boolean validateMD5Mac(String md5from, String[] seeds) {
		boolean theSame = false;
		try {
			if (StringUtils.isBlank(md5from)) {
				return false;
			}
			if (seeds == null || seeds.length == 0) {
				return false;
			}
			String md5to = DigestUtils.md5Hex(StringUtils.join(seeds, ""));
			return md5from.trim().equalsIgnoreCase(md5to);
		} catch (Exception e) {
			theSame = false;
		}
		return theSame;
